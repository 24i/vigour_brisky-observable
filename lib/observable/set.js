...
exports.define = {
  handleReference (val, stamp, oldVal) {
    ...
    if (valIsObservable) {
      // just call the set i sbetter
      // make this more efficient e.g. just do a set
      // use uid or sid
      // something like this...
      val.emitters.set({ data: { [this.uid()]: this } }, stamp)
      //  on (type, val, key, stamp) {
      this.val = val.on('data', this, void 0, stamp)
    }
